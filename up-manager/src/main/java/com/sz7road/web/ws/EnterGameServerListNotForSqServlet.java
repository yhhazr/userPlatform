 ...
 */
@Singleton
public class EnterGameServerListNotForSqServlet extends BaseServlet{

    private static final Logger log = LoggerFactory.getLogger(EnterGameServerListNotForSqServlet.class.getName());
    ...
    public void getServerList(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (_gid <= 0) {
            ...
            try {
                Set<ServerTableNotForSqEntity.ServerEntryNotForSqEntity> serverEntrySet = Sets.newTreeSet(new Comparator<ServerTableNotForSqEntity.ServerEntryNotForSqEntity>() {
                    @Override
                    public int compare(ServerTableNotForSqEntity.ServerEntryNotForSqEntity o1, ServerTableNotForSqEntity.ServerEntryNotForSqEntity o2) {
                ...
                Collection<ServerTableNotForSqEntity.ServerEntryNotForSqEntity> serverEntries = gameWithServerService.getSortedServerEntriesNotForSq(_gid);
                Iterator<ServerTableNotForSqEntity.ServerEntryNotForSqEntity> it = serverEntries.iterator();
                while (it.hasNext()) {
                    ServerTableNotForSqEntity.ServerEntryNotForSqEntity serverEntry = it.next();
                    Timestamp current=new Timestamp(System.currentTimeMillis()+100000);
                    int status=serverEntry.getStatus();
                    //返回火爆或者维护，并且开服时间在之前100秒的。
                    if (serverEntry.getOpenTime().before(current) &&(status== SERVER_HOT_STATUS || status ==SERVER_MAINTAIN_STATUS)) {
