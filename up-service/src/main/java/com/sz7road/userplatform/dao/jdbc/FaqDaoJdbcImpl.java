...
import com.sz7road.userplatform.dao.FaqDao;
 ...
 * 常见问题后台的管理实现
 */
public class FaqDaoJdbcImpl extends JdbcDaoSupport<FaqObject> implements FaqDao {
    ...
    public JqObject queryFaq(Map conditions, int start, int pageSize, String sortFiled, String sortOrder) {
        ...
        if (countAndResultsql != null && !countAndResultsql.isEmpty()) {
            try {
                ...
                jqObject.setTotal(DataUtil.getTotalPages(count, pageSize));
    ...
    public JqObject queryFaqById(int id) {
        ...
        if (true) {
            try {
                ...
                jqObject.setTotal(DataUtil.getTotalPages(count, 10));
                jqObject.setRecords(count);
                jqObject.setPage(1);
                //拿到数据
