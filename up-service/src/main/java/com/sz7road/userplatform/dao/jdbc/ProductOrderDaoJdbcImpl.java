...
public class ProductOrderDaoJdbcImpl extends JdbcDaoSupport<ProductOrder> implements ProductOrderDao
{
    ...
    protected String getTableName() {
        return "`" + TableLocator.getProductOrderTable() + "`";
    }

    @Override
    public int add(final ProductOrder order) throws SQLException {
        if (null == order) {
    ...
    @Override
    public ProductOrder getByOrderId(String order) throws SQLException {
        final QueryRunner runner = getQueryRunner();
        String SQL = QUERY_TEMPLATE + " AND `id`=?";
        SQL = SQL.replace("$fields", "*");
        SQL = SQL.replace("$table", getTableName());

        return runner.query(SQL, new BeanHandler<ProductOrder>(ProductOrder.class), order);
    }

    @Override
    public int update(ProductOrder order) throws SQLException {
        if (null == order)
