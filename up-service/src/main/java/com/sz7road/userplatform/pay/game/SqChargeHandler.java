...
@RequestScoped
public class SqChargeHandler implements GameRechargeHandler{

    static final Logger log = LoggerFactory.getLogger(SqChargeHandler.class.getName());
    ...
    public Backend.BackendResponse charge(GameTable.GameEntry gameEntry, ServerInfo serverInfo, ServerInfo mainServerInfo, OrderObject order) {
        ...
        // 获取充值KEY
        final String aliasName = "sq";
        final String chargeKey = ConfigurationUtils.get(aliasName + ".chargeKey");
        //final String chargeHashKey = aliasName + ".chargeKey";
        //chargeKey = GameWithServerService.getServerValueByKey(serverEntry, chargeHashKey);

        if (Strings.isNullOrEmpty(chargeKey)) {
            throw new IllegalStateException("神曲无效的游戏充值KEY");
        }
        ...
        if (userId > 0 && pid > 0 && amount > 0) {
            ...
            //final String hashKey = "sq.chargeUrl";
            //final String hashKey = aliasName + ".chargeUrl";
            //String dest = GameWithServerService.getServerFormatValueByKey(serverEntry, hashKey);
            ...
            throw new IllegalStateException("要求发放无效充值订单游戏币:" + order);
        }
        return response;
    }
