        ...
        try {
            ...
            if (payBean.isAvailableForSubmit()) {
                ...
                if (null != account && null != (order = payService.submitOrder(account, payBean))) {
                    ...
                    postBean.setTotalFree(payBean.getPayAmount()/100);
                    ...
                    postBean.setShowUrl(ConfigurationUtils.get("ppay.alipay.showUrl"));
                    postBean.setReturnUrl(getAssertUrl(ConfigurationUtils.get("ppay.alipay.returnUri")) + payBean.getChannelTag());
                    postBean.setNotifyUrl(getAssertUrl(ConfigurationUtils.get("ppay.alipay.notifyUri")) + payBean.getChannelTag());
                    postBean.setSubject(getProductName(payBean));
                    postBean.updateSign();
        ...
        try {
            ...
            if (mysign.equals(sign) && responseTxt.equals("true")) {
                if (callbackBean.getTradeStatus().equals("TRADE_FINISHED") || callbackBean.getTradeStatus().equals("TRADE_SUCCESS")) {
                    // 进行处理
                    final ProductPayService payService = getInstance(ProductPayService.class);
                    final ProductOrder orderObject = payService.getOrder(callbackBean.getOrder());
