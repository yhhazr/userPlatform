            ...
            <div class="getPwdBody">
                ...
                <div id="inputMsg" class="inputMsg">
                    <div id="tabs" class="tabs">
                        <ul class="ul_tabs">
                            <li id="item-1" class="tb-item">密码找回方式</li>
                        </ul>
                    </div>
                    ...
                    <div class="getPwdMain" id="getPwd_item_1">
                        <div class="getContent">
                            <form id="findPwdForm" name="findPwdForm" method="post">
                                <%--
                                <p>密保邮箱：<a class="codeText" href="FindPwdWaySubmit?userName=${userName}&code=${code}&verify=${verify}&type=email">找回密码</a></p>
                                --%>
                                <p>密保邮箱：<a class="codeText" href="javascript:;" onclick="sendMail();">找回密码</a></p>

                                <p>密保问题：<a class="codeText" href="FindPwdWaySubmit?userName=${userName}&code=${code}&verify=${verify}&type=question">找回密码</a></p>

                                <p>密保手机：<a class="codeText" href="FindPwdWaySubmit?userName=${userName}&code=${code}&verify=${verify}&type=mobile">找回密码</a></p>

                                <p>账户申诉：<a class="codeText" href="FindPwdWaySubmit?userName=${userName}&code=${code}&verify=${verify}&type=appeal">找回密码</a></p>

                                <p style="margin:35px 0px;"><span class="btn_next" id="btn_pre_sub">上一步</span></p>
                            </form>
                        </div>
...
<script type="text/javascript">
    ...
    function sendMail(){
        $.ajax({
            ...
            dataType:"JSON",
            url:"/ForgetPwdSubmit",
            data:{_u:"${userName}", _v:"${verify}", _c:"${code}"},
            beforeSend:function (XMLHttpRequest, textStatus) {

            },
            success:function (msg) {
                if (msg.code > 0) {
                    var html = msg.msg + '<a href="FindPwd">返回</a>';
                    $('#inputMsg').hide();
                    $('#sendMsg').show().html(html);
                }
                else {
                    var html = '您的申请已提交成功，请查看您的<span class="user_email">' + msg.object + '</span>邮箱。<br>注意:请您在收到邮件1个小时内使用，否则该链接将会失效。';
                    var btnHtml = '<p style="margin:35px 160px;"><span class="btn_next" id="btn_close_sub" onclick="window.close();"> 关闭本页 </span></p> ';
                    $('#inputMsg').hide();
                    $('#sendMsg').show().html(html);
